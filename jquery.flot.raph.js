...
(function($) {
    function Plot(placeholder, data_, options_, plugins) {
        ...
        plot.getCanvas = function() { return paper; };
            ...
            function plotLine(datapoints, xoffset, yoffset, axisx, axisy) {
                ...
                for (var i = ps; i < points.length; i += ps) {
                    ...
                    if (x1 != prevx || y1 != prevy)
                        path += "M"+(axisx.p2c(x1) + xoffset)+" "+(axisy.p2c(y1) + yoffset);
        ...
        function drawBar(x, y, b, barLeft, barRight, offset, fillStyleCallback, axisx, axisy, c, horizontal, lineWidth) {
            ...
            if (fillStyleCallback) {
                currentSet.push(paper.rect(left, top, right-left, bottom-top).attr({
                    "fill":fillStyleCallback(bottom, top),
                    "stroke":null
                }));
            }
            ...
            if (lineWidth > 0 && (drawLeft || drawRight || drawTop || drawBottom)) {
                ...
                path += "M"+left+" "+ (bottom + offset);
                if (drawLeft) {
                    path += "L"+left+" "+ (top + offset);
                } else {
                    path += "M"+left+" "+ (top + offset);
                }
                if (drawTop) {
                    path += "L"+right+" "+ (top + offset);
                } else {
                    path += "M"+right+" "+ (top + offset);
                }
                if (drawRight) {
                    path += "L"+right+" "+ (bottom + offset);
                } else {
                    path += "M"+right+" "+ (bottom + offset);
                }
                if (drawBottom) {
                    path += "L"+left+" "+ (bottom + offset);
                } else {
                    path += "M"+left+" "+ (bottom + offset);
                }
                currentSet.push(paper.path(path));
        ...
        function drawSeriesBars(series) {
            function plotBars(datapoints, barLeft, barRight, offset, fillStyleCallback, axisx, axisy) {
                ...
                for (var i = 0; i < points.length; i += ps) {
                    if (points[i] == null)
                        continue;
                    drawBar(points[i], points[i + 1], points[i + 2], barLeft, barRight, offset, fillStyleCallback, axisx, axisy, null, series.bars.horizontal, series.bars.lineWidth);
                }
            }
